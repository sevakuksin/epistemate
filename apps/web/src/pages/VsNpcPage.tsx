...
import { applyMove, createGameFromSetup, generatePseudoLegalMoves, serializeGame } from "@cv/engine";
...
export function VsNpcPage() {
  ...
  /** Default: White (you) toward bottom; manual “Flip board” only — no auto-rotate when the AI moves. */
  ...
  useEffect(() => {
    ...
    if (state.sides[state.currentTurnIndex] !== "black") return;

    const seq = ++npcSeqRef.current;
    const frozen = state;
    setNpcThinking(true);
    const w = new Worker(new URL("../workers/npc.worker.ts", import.meta.url), { type: "module" });
    w.postMessage({ serialized: serializeGame(frozen), timeMs: 2500, maxDepth: 8 });
    w.onmessage = (e: MessageEvent<{ bestMove?: CompactMove | null; score?: number; error?: string }>) => {
      if (seq !== npcSeqRef.current) {
        w.terminate();
        return;
      }
      ...
      const m = e.data.bestMove;
      if (!m) return;
      try {
        const next = applyMove(frozen, m);
        setState(next);
        ...
        setError("AI could not apply its move.");
      }
    };
    w.onerror = () => {
      if (seq !== npcSeqRef.current) return;
      setNpcThinking(false);
  ...
  function onSquareClick(x: number, y: number) {
    if (!state || state.status !== "ongoing") return;
    if (state.sides[state.currentTurnIndex] !== "white") return;
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Vs NPC</h1>
          <p className="subtitle">You play White; the AI plays Black (same-device, in-browser search).</p>
        </div>
        ...
        <div className="row game-layout" style={{ alignItems: "flex-start" }}>
          ...
          <div className="game-sidebar" style={{ minWidth: 280 }}>
            <div className="card">
              ...
              {npcThinking ? <p className="subtitle">AI is thinking…</p> : null}
